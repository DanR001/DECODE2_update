...
#---for output
#outfile1 = './OUTPUT_TREE_EXAMPLE/tree%i_lgM%.2f.npz' #%(itree,lgM0)
...
#def loop(itree):
def loop(redshift_array, Mhalo_assembly_history):
    """
    Replaces the loop "for itree in range(Ntree):", for parallelization.
    """

    #---target halo and desired resolution
    lgM0 = Mhalo_assembly_history[0] #13. #- np.log10(cfg.h) # log10(Msun), corresponds to 10^14.2 Msun/h
    cfg.psi_res = 10**-3.0
    lgMres = lgM0 + np.log10(cfg.psi_res) # psi_{res} = 10^-5 by default
    lgMres = np.max([lgMres, 11.])
    ...
    # as the current branch, before moving on to the next-level branches.
        ...
        z = np.array(z)
        #if ik==0:
        #    M = 10.**interp1d(redshift_array,Mhalo_assembly_history,fill_value="extrapolate")(z)
